...
;;
(defmethod log :file [{from :from to :to {filename :name} :file}]
...
(defmulti route (fn [req] ((juxt :uri :method) req)))
...
; Implementation using defrecord - my first approach
(comment (defrecord B [alcohol sugar carbs]
  Beverage
  (->map [beverage]
    {:alcohol alcohol :sugar sugar :carbs carbs})))

(comment (defn serve [alcohol sugar carbs]
  (B. alcohol sugar carbs)))

; Now using reify - much simpler
